<title>Photo+ Subscription</title>

<?php 
function subUser($Bdd) {
    if (isset($_SESSION["newSub"])) {
        $sub = $_SESSION["newSub"];
        $expDate = date('Y-m-d H:i:s', strtotime('+31 days'));
        $id = $_SESSION["userid"];
        $req = "UPDATE users SET sub='$sub' WHERE id = '$id'";
        $ORes = $Bdd->query($req);
        $_SESSION['sub'] = $sub;
...
if (isset($_POST["goBack"])) {
    ...
    subUser($Bdd);
} else if (isset($_POST["goldSubChoosen"]) || isset($_POST["plusSubChoosen"])) {
    $_SESSION["willCard"] = true;
    if (isset($_POST["goldSubChoosen"])) $_SESSION["newSub"] = "goldSub";
    else $_SESSION["newSub"] = "plusSub";
}
...
if (!isset($_SESSION["willCard"])) {
    if ($_SESSION["userid"]) {
        ...
        if($Usr = $ORes->fetch()) {
            ...
            ?> 
            <div class='flex flex-col justify-center items-center w-full mt-1'>
                <p class='font-bold text-2xl'>Subscription chooser</p>
                <?php 
                if ($sub == 3) {
                    $goldChoosen = 'shadow-2xl shadow-yellow-500/50';
                    $plusChoosen = '';
                    $freeChoosen = '';
                    $datef = explode(" ", $Usr->subexp)[0];
                    $d1 = strtotime($datef);
                    $d2 = time();
                    $date = round(($d1 - $d2) / (60 * 60 * 24));
                    echo "<p class='font-semibold text-lg text-yellow-500'>Current: GOLD ($datef)</p>";
                    echo "<p class='font-semibold text-lg text-yellow-500'>$date days remaning!</p>";
                } else if ($sub == 2) {
                    $goldChoosen = '';
                    $plusChoosen = 'shadow-2xl shadow-yellow-500/50';
                    $freeChoosen = '';
                    $datef = explode(" ", $Usr->subexp)[0];
                    $d1 = strtotime($datef);
                    $d2 = time();
                    $date = round(($d1 - $d2) / (60 * 60 * 24));
                    echo "<p class='font-semibold text-lg text-green-300'>Current: Photo+ ($datef)</p>";
                    echo "<p class='font-semibold text-lg text-green-300'>$date days remaning!</p>";
                } else {
                    $goldChoosen = '';
                    $plusChoosen = '';
                    $freeChoosen = 'shadow-2xl shadow-yellow-500/50';
                    echo "<p class='font-semibold text-lg text-gray-500'>Current: FREE</p>";
                }
                ?>
            </div>
            <section class="flex flex-col lg:flex-row items-start items-center lg:justify-center w-full w-full lg:px-10 py-12 ">
                ...
                <article class="w-4/5 lg:w-custom mb-10 lg:px-4 py-10 text-center text-primary-dark bg-[#defade] <?php echo $freeChoosen; ?>">
                    <div class='border-b border-gray-300 h-[7rem]'>    
                    ...
                    <?php
                    if ($sub == 1) {
                        echo "<p class='h-[1px] my-7 font-semibold'>CURRENT</p>";
                    } else {
                        echo "<p class='h-[1px] my-7 font-semibold'>CAN'T SUBSCRIBE</p>";
                    }
                    ?>
                </article>
                <article class="w-4/5 lg:w-custom mb-10 h-[27rem] rounded-l-lg lg:px-4 py-10 text-center text-primary-dark bg-[#a7f0a6] <?php echo $plusChoosen; ?>">
                    <div class='border-b border-gray-300 h-[7rem]'>
                    ...
                    <?php
                    if ($sub == 2) {
                        echo "<p class='h-[1px] my-9 font-semibold'>CURRENT</p>";
                    } else if ($sub == 3) {
                        echo "<p class='h-[1px] my-9 font-semibold'>CAN'T SUBSCRIBE</p>";
                    } else {
                        ?>
                ...
                <article class=" bg-[#50c74f] w-4/5 h-[28rem] lg:w-custom mb-10 lg:px-4 py-10 text-center text-primary-dark rounded-lg <?php echo $goldChoosen; ?>">
                    <div class='border-b border-gray-300 h-[7rem]'>
                    ...
                    <?php
                    if ($sub == 3) {
                        echo "<p class='h-[1px] my-10 font-semibold'>CURRENT</p>";
                    } else {
                        ?>
                        <form action="" method="POST">
                            <button name='goldSubChoosen' class="bg-white rounded-md shadow-md mt-7 p-2 hover:bg-green-300 hover:mt-[2rem] hover:p-1 transition-all duration-500">
                                Subscribe
                            </button>
                        </form>
                        <?php
